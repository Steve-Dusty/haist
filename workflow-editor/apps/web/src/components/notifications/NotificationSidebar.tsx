...
export function NotificationSidebar({ isOpen, onClose }: NotificationSidebarProps) {
  ...
  return (
    <aside
      className={clsx(
        "h-screen bg-card/50 backdrop-blur-sm border-l border-border/50 flex flex-col shrink-0 transition-[width] duration-150 ease-out overflow-hidden",
        isOpen ? "w-80" : "w-0"
      )}
    >
      {/* Header */}
      <div className="flex items-center justify-between px-4 h-14 border-b border-border/50 shrink-0">
        <h2 className="text-sm font-semibold text-foreground whitespace-nowrap">Notifications</h2>
        <div className="flex items-center gap-1">
          <button
            onClick={markAllRead}
            className="inline-flex items-center gap-1 px-2 py-1 text-xs text-muted-foreground hover:text-foreground hover:bg-muted rounded-md transition-colors whitespace-nowrap"
            type="button"
          >
      ...
      <div className="flex-1 overflow-y-auto overflow-x-hidden">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-12 text-muted-foreground">
            ...
            <p className="text-sm whitespace-nowrap">No notifications yet</p>
          </div>
        ) : (
          <>
            {notifications.map((n) => {
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      ...
                      <button
                        ...
                        className="text-[10px] text-muted-foreground/50 hover:text-destructive ml-auto whitespace-nowrap"
                        type="button"
                      >
                        Delete
                      </button>
